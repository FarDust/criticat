...
class FormatIssue(BaseModel):
    description: str = Field(
        ..., description="Brief description of the formatting issue."
    )
    explanation: str = Field(..., description="Explanation of the issue.")
    example: str = Field(
        "", description="Example or snippet from the document."
    )  # No Optional
    cause: str = Field(
        "", description="Likely cause of the issue (e.g., LaTeX misconfig)."
    )
    status: Literal["warning", "error", "info"] = Field(
        ...,
    ...
    confidence: int = Field(
        ..., description="Confidence level from 1 to 5.", ge=1, le=5
    )
